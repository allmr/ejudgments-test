<script>
  ...
  $: isHeadnoteOpen =
    headnoteValue === "" || hnAutoFill.length === 0 ? false : true;

  $: headnoteUlClass = !isHeadnoteOpen
    ? "autocomplete-results hide-results"
    : "autocomplete-results ";

  $: if (headnoteValue !== "") {
    autoFill(headnoteValue);
  }
  const dispatch = createEventDispatcher();

  const setHeadnoteValue = (event) => {
    headnoteValue = event.target.id;
  };

  const addHeadnote = () => {
    let result = headnoteValue;
    headnoteValue = "";
    dispatch("headnoteData", {
      value: result,
    });
  };
...
<div class="w-4/5 mr-1">
  <input
    ...
    placeholder="Search in head note"
    bind:value={headnoteValue}
  />
</div>
<div class="w-1/5">
  <button
    ...
    on:click={addHeadnote}
    >ADD
  </button>
</div>
{#if isHeadnoteOpen}
  <ul class={headnoteUlClass}>
    {#each hnAutoFill as item}
      <li
        class="bg-gray-100 hover:bg-gray-300 px-2 py-2 text-lg"
        id={item.view}
        on:click={setHeadnoteValue}
      >
        {item.view}
      </li>
    {/each}
